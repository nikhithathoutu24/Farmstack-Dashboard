...
if not st.sidebar.checkbox("Yield quantities based on region", True, key='6'):
    select_region=st.sidebar.selectbox('region',['India','Morocco','Russian Federation'],key=7)
...
if not st.sidebar.checkbox("Value of Agriculture production", True, key='4'):
    select_region=st.sidebar.selectbox('region',['India','Morocco','Russian Federation'],key=8)
    select_item=st.sidebar.selectbox('Item',items,key=9)
    data=load_valueData()
    x=data[data["Item"]==select_item]
...
if not st.sidebar.checkbox("Mean Temperature Change ", True, key='5'):
    select_region=st.sidebar.selectbox('region',['India','Morocco','Russian Federation'],key=10)
    select_time=st.sidebar.selectbox('month',month,key=11)
